...
from mmhuman3d.utils.demo_utils import box2cs, prepare_frames, xywh2xyxy, xyxy2xywh
from mmhuman3d.utils.ffmpeg_utils import array_to_images
from mmhuman3d.utils.transforms import Compose, rotmat_to_aa
...
def custom_render(verts,camera,batch_size,device,image_array=None):
    ...
    #     bboxes_xyxy.append([0,0,224,224])
...
def diff_render_test(args,frames_iter,results=None):
    ...
    batch_size = len(frames_iter)

    # new model forward

    np_results = results
    results = results['gd_preds']
    for k,v in results.items():
        if isinstance(v,torch.Tensor):
            print(f"{k} shape:{v.shape} requires_grad:{v.requires_grad}")

    render_tensor = custom_render(results['vertices'],results['camera'],batch_size,device)
...
python demo/estimate_smpl_custom_process.py \
    ...
    --show_path vis_results/custom_process_demo_image/ \
    --output demo_result
"""